...
- **Secrets: local secret store, injected via environment variables.** The
  OAuth client id/secret and the oauth2-proxy cookie secret live in a local
  store (e.g. `pass`, `sops`+age, a keychain, or a git-ignored `.env` file with
  `0600` perms) and are exported into the process environment
  (`OAUTH2_PROXY_CLIENT_ID`, `OAUTH2_PROXY_CLIENT_SECRET`,
  `OAUTH2_PROXY_COOKIE_SECRET`). Cloud secret managers are deferred to the
  cloud path.
- **Tailscale auth**: the local machine can be brought onto the tailnet with a
...
- `client_id` / `client_secret` — from a Google OAuth 2.0 Web client (manual
  step below); supplied via env vars from the local secret store
...
1. **Google OAuth client** — in the GCP console, create an OAuth consent screen
   ...
   without Funnel). Copy the client id/secret into the local secret store.
2. **Cookie secret** — `openssl rand -base64 32`, stored alongside the OAuth
   secret in the local store.

## Planned folder layout

```
agent-coordinator/web-exposure-poc/
  INITIAL-SETUP.md          # this document (localhost path)
  INITIAL-CLOUD.md          # cloud-compute path, later increment
  terraform/
    auth/                   # project + API enablement for the Google OAuth client
  config/
    oauth2-proxy.cfg        # non-secret settings
    .env.example            # names of the env vars pulled from the local secret store
    agent-coordinator.service / oauth2-proxy.service   # systemd --user units
    run.sh                  # start the chain without systemd
```

## Bring-up sequence

1. `terraform apply` in `terraform/auth/` to create/enable the Google project.
2. Manual: create the consent screen + OAuth Web client; generate the cookie
   secret; put client id, client secret, and cookie secret into the local
   secret store; copy `config/.env.example` and fill it in.
...
4. Start AC on `127.0.0.1:8083`; start oauth2-proxy on `127.0.0.1:4180` with
   the env vars exported from the local store.
