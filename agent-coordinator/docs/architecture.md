...
| LR → AC | `data` / `"files-state"` | `FilesStateMsg` — host-cache listing for the files tab (read-only on AC; see [DistributedExchange.md](../../docs/DistributedExchange.md)) |
...
relays to browsers as `lr-files-state` for a read-only view of the tab.

Upload stays a **direct-LR-client-only** operation this increment: AC's
`proxyToHost` reverse proxy stamps every request it forwards with
...
| `lr-files-state` | `{ host_id, active, files[]? }` | Host's files tab listing (read-only; AC has no upload affordance) |
