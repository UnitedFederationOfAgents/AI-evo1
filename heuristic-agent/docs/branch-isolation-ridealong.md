...
This ridealong demonstrates the complete branch-isolation workflow using heuristic-agent. The flow allows an agent to work on a repository branch without having direct git access - the `.git` directory is kept secure and only merged back when the agent's work is complete.
...
- heuristic-agent is built: `make build`
...
```bash
./heuristic-agent watch --agent-type agent-worker > /tmp/heuristic-agent-watch.log 2>&1 &
```
...
```ridealong
./heuristic-agent readspace repo clone UnitedFederationOfAgents/AI-evo1
```

Clone into writespaces (for modifications, `.git` will be moved to secure storage):

```ridealong
./heuristic-agent writespace repo clone UnitedFederationOfAgents/AI-evo1
```
...
```ridealong
./heuristic-agent slopspace add-readspace repo "$SLOP_ID" UnitedFederationOfAgents/AI-evo1
```

Add the repo to writespaces, creating the new branch (`.git` moved to secure storage):

```ridealong
./heuristic-agent slopspace add-writespace repo "$SLOP_ID" UnitedFederationOfAgents/AI-evo1 --ref "$BRANCH"
```
...
{"id":"branch-iso-${TS}","work_type":"slopspace","agent_type":"agent-worker","role":"branch-isolation-ridealong","prompt":"Our nice agent should create the file writespaces/repos/UnitedFederationOfAgents/AI-evo1/heuristic-agent/docs/ridealong-result.md","agent":"clod","model":"sonnet","status":"pending","created_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ)","updated_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ)"}
EOF
...
```ridealong
cat /tmp/heuristic-agent-watch.log
```
...
```ridealong
cat "/host-agent-files/slopspaces/$SLOP_ID/writespaces/repos/UnitedFederationOfAgents/AI-evo1/heuristic-agent/docs/ridealong-result.md" 2>/dev/null || echo "File not found - check watch log above"
```
...
```ridealong
./heuristic-agent slopspace write "$SLOP_ID" all --message "feat: add ridealong result from branch-isolation demo"
```
...
```ridealong
./heuristic-agent readspace repo delete UnitedFederationOfAgents/AI-evo1
./heuristic-agent writespace repo delete UnitedFederationOfAgents/AI-evo1
```
