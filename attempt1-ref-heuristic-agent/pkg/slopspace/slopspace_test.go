...
func TestManagerCreate(t *testing.T) {
	...
	metadata, err := mgr.Create(types.AgentTypeWorker)
	if err != nil {
	...
	if metadata.AgentType != types.AgentTypeWorker {
		t.Errorf("expected agent type %s, got %s", types.AgentTypeWorker, metadata.AgentType)
	}
...
func TestManagerGetAndList(t *testing.T) {
	...
	metadata1, err := mgr.Create(types.AgentTypeWorker)
	if err != nil {
		t.Fatalf("failed to create slopspace 1: %v", err)
	}

	metadata2, err := mgr.Create(types.AgentTypeHeuristic)
	if err != nil {
...
func TestManagerDeployAndReturn(t *testing.T) {
	...
	// Deploy
	if err := mgr.Deploy(metadata.ID); err != nil {
	...
	deployedFile := filepath.Join(cfg.DeployPath(), WriteSpacesDir, "test.txt")
	if _, err := os.Stat(deployedFile); os.IsNotExist(err) {
		t.Error("expected test file to be deployed")
	}

	// Check marker file
	markerFile := filepath.Join(cfg.DeployPath(), "SLOPSPACE_ID")
	...
	// Check GetDeployedID
	deployedID, err := mgr.GetDeployedID()
	if err != nil {
