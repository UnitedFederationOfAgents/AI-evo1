...
`--auto-connect --remote` (from a terminal it hosts for you — see
`local-representative`'s README). `--remote` implies `--auto-connect`, and it
makes the completed background connect land in **remote control** — the terminal
shows the shell but LR drives it — so a chain that begins with a single
`./local-representative` comes up fully ready to operate from the dashboard.
