// Package readspace handles readspace lifecycle management for repository clones.
package readspace
...
const (
	// ReposDir is the subdirectory for repository readspaces.
	ReposDir     = "repos"
	MetadataFile = "READSPACE.json"
)

// RepoMetadata stores information about a cloned repository readspace.
...
// Manager handles readspace lifecycle operations.
type Manager struct {
	config *types.Config
}

// NewManager creates a new readspace manager.
func NewManager(cfg *types.Config) *Manager {
	return &Manager{config: cfg}
}

// ReadspacesDir returns the base readspaces directory.
func (m *Manager) ReadspacesDir() string {
	// Readspaces dir is beside slopspaces dir
	return filepath.Join(filepath.Dir(m.config.SlopspacesDir), "readspaces")
}

// ReposPath returns the path to the repos subdirectory.
func (m *Manager) ReposPath() string {
	return filepath.Join(m.ReadspacesDir(), ReposDir)
}

// RepoPath returns the path to a specific repo readspace.
func (m *Manager) RepoPath(owner, repo string) string {
	return filepath.Join(m.ReposPath(), owner, repo)
}

// CloneRepo clones a repository into the readspaces directory.
...
func (m *Manager) CloneRepo(ownerRepo string) (*RepoMetadata, error) {
	...
	// Clone using gh with PAT authentication
...
// Get retrieves metadata for a repo readspace.
...
// List returns all repo readspaces.
...
// Delete removes a repo readspace.
func (m *Manager) Delete(ownerRepo string) error {
	...
	if _, err := os.Stat(repoPath); os.IsNotExist(err) {
		return fmt.Errorf("repo readspace does not exist: %s", ownerRepo)
	}

	if err := os.RemoveAll(repoPath); err != nil {
		return fmt.Errorf("failed to delete repo readspace: %w", err)
	}
...
// readMetadata reads repo readspace metadata from disk.
...
// writeMetadata writes repo readspace metadata to disk.
