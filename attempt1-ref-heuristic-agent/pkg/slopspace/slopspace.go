...
// Create creates a new slopspace with the given agent type.
func (m *Manager) Create(agentType types.AgentType) (*types.SlopspaceMetadata, error) {
	id := uuid.New().String()
	now := time.Now()
	...
	metadata := &types.SlopspaceMetadata{
		Slopspace: types.Slopspace{
			...
			CreatedAt:  now,
			RootPath:   rootPath,
			DeployPath: filepath.Join(m.config.DeployPath(), id),
			AgentType:  agentType,
			Deployed:   false,
		},
		Iteration: 0,
...
// Deploy moves the slopspace contents to the agent workspace.
...
func (m *Manager) Deploy(id string) error {
	...
	deployPath := m.config.DeployPath()
...
// GetDeployedID returns the ID of the currently deployed slopspace, if any.
func (m *Manager) GetDeployedID() (string, error) {
	markerPath := filepath.Join(m.config.DeployPath(), "SLOPSPACE_ID")
